...
class MLPLayer(nn.Module):
    def __init__(self, input_dim, hidden_size) -> None:
        ...
        self.fc = nn.Sequential(*fc_h) # 等效于nn.Sequential(fc_h[0],fc_h[1])
         # 输入12维 → Linear(12,128) → ReLU → LayerNorm(128) → Linear(128,128) → ReLU → LayerNorm(128)
...
class BaselineActor(nn.Module):
    def __init__(self, input_dim=12, use_mlp_actlayer=False) -> None:
        super().__init__()
        self.tpdv = dict(dtype=torch.float32, device=torch.device('cpu')) # 存储数据类型和设备信息
    ...
    def check(self, input):
        output = torch.from_numpy(input) if type(input) == np.ndarray else input # 从numpy.ndarray创建一个张量。返回的张量和numpy.ndarray共用内存
        return output

    def forward(self, obs, rnn_states):
        x = check(obs).to(**self.tpdv) # to()将张量、模型转换为指定设备（如CPU或GPU）可用的格式;**操作符将字典展开为关键字参数
        h_s = check(rnn_states).to(**self.tpdv)
        x = self.base(x)
        x, h_s = self.rnn(x, h_s)
        actions = self.act(x)
        return actions, h_s
    # obs(12维)→2层MLP(128，128)→1层GRU(128)→1层ACTLayer→actions
    # rnn_states→GRULayer→h_s